...
from Cad_VLM.models.text2cad import CSG2CAD
...
@logger.catch()
def main():
    print_with_separator("😊 CSG2CAD Training 😊")
    ...
    text2cad = CSG2CAD(text_config=config["text_encoder"], cad_config=cad_config).to(
        device
    )
...
def train_model(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        with tqdm(
            train_loader,
            ascii=True,
            desc=f"\033[94mCSG2CAD\033[0m: Epoch [{epoch}/{num_epochs+1}]✨",
        ) as pbar:
